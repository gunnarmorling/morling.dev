...
Ideas for future development include a more powerful "built-in" API which e.g. provides the functionality for calculating the total TLAB allocations of a given set of threads as a ready-to-use method.
...
The JFR event streaming API could also be leveraged for streaming queries on live events streamed from a remote system.
