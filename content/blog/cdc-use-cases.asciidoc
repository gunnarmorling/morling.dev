...
_This post originally appeared on the link:https://www.decodable.co/blog/cdc-use-cases[Decodable blog]. All rights reserved._
...
.Application-side caches in a distributed service, kept in sync with the primary DB via CDC
...
.Enriching CDC events with transactional metadata using stream processing
...
.Continuous query for incrementally computing a join between two CDC streams
