...
_This post originally appeared on the link:https://www.decodable.co/blog/change-data-capture-breaks-encapsulation-does-it-though[Decodable blog]._

Having worked on Debezium—an open-source platform for Change Data Capture (CDC)—for several years, one concern I’ve heard repeatedly is this: aren’t you breaking the encapsulation of your application when you expose change event feeds directly from your database?
...
=== CDC—A Quick Primer
With log-based change data capture—for instance, using  link:https://debezium.io/[Debezium] —you can expose realtime change event streams for the tables of your database, sourced from the database’s transaction log.
...
These event streams enable a  link:https://www.decodable.co/blog/seven-ways-to-put-cdc-to-work[large variety of use cases] , such as low-latency data feeds for analytical data stores, cache updates, full-text search indexes, and many more.
...
Specifically, do change events permeate multiple  link:https://martinfowler.com/bliki/BoundedContext.html[bounded contexts]  (in terminology of  link:https://en.wikipedia.org/wiki/Domain-driven_design[Domain-Driven Design] )?_
_Are they propagated across system (and team) boundaries, or not?
...
But also a more formal, machine-readable contract definition is possible, such as  in the form of YAML (_will this ever_ link:https://twitter.com/search?q=yaml%20(from:brunoborges)%20min_retweets:25&src=typed_query[stop] _?!_), as suggested by the  link:https://datacontract.com/[Data Contracts specification] .
...
If you are on Postgres, an interesting implementation option is  link:https://www.decodable.co/blog/the-wonders-of-postgres-logical-decoding-messages-for-cdc[logical decoding messages] : instead of having a bespoke outbox table, Postgres lets you write arbitrary events solely to its write-ahead log, from where they can be retrieved using CDC.
...
----
*Listing 1: Configuration of the Postgres source connector.*
...
----
*Listing 2: Configuration of the Kafka sink connector.*
...
INSERT INTO customers_public
  ...
  FROM customers;
----
*Listing 3: Flink SQL job for transforming the source table into the sink table.*
...
As an example, Postgres supports the definition of column lists and  link:https://www.decodable.co/blog/postgres-15-logical-decoding-row-filters-with-debezium[row filters] , providing fine-grained control over the contents of any logical replication streams, thus helping to reduce network traffic and any potential cost associated with it.
...
----
*Listing 4: Joining two source change streams into a single public stream.*
...
INSERT INTO customers_public
  ...
  FROM customers;
----
*Listing 5: Retrieving the first name from the correct column, depending on which value is present*
