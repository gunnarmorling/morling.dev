...
For Postgres, you also can track the retained WAL size of a replication slot using the `pg_current_wal_lsn()` and `pg_wal_lsn_diff()` functions, as I described in https://www.infoq.com/articles/wonders-of-postgres-logical-decoding-messages/[this blog post] a while ago.
