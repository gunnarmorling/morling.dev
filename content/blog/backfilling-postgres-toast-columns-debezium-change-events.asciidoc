...
The `LAST_VALUE()` aggregation function propagates the last non `NULL` value for the given group.
...
The data is partitioned by id, so that the aggregation happens in the context of each individual row of the input table.
The aggregation is wrapped by a `CASE` clause in order to handle the situation where the TOAST-ed column actually is set to `NULL`.
...
which are typically applied for analytical use cases.
Nevertheless, solving the problem with just a few lines of SQL makes for a very elegant solution,
especially when considering that Flink SQL tends to be more widely supported by SaaS vendors than the DataStream API,
