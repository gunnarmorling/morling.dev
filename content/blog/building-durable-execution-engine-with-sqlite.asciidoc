...
A DE engine transparently tracks program progress, persists execution state, and enables resumption after failures.
...
Examples for this include https://temporal.io/[Temporal], https://www.restate.dev/[Restate], and https://www.inngest.com/[Inngest].
The other option is to persist DE state in the local database of a given application or (micro)service.
One example in this category is https://www.dbos.dev/[DBOS], which implements DE on top of Postgres.
...
This log table contains all information needed to capture execution intent and persist results.
...
allowing to express that flow a bit more concisely:
...
Thanks to Java's dynamic nature, this is relatively easy, requiring just a little bit of bytecode generation.
...
which lets the invoked services detect and ignore duplicate calls.
...
All it does is persisting step invocations and their status in the `execution_log` SQLite table shown above.
...
Instead, we need a way to yield continuation of the flow, return control to the caller,
...
Note that it is essential that no user-provided flow or step code catches that exception type.

This works, but Java 21 offers a simpler solution.
...
At the persistence level, you'd have to address aspects such as  fault tolerance and HA.
