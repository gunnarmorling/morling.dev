...
By measuring and asserting metrics that _impact_ an application's performance characteristics, most notably latency and throughput,
rather than examining those primary metrics themselves,
you can implement stable and reliable automated tests,
whose outcome does not depend on the capabilities of the execution environment (e.g. number/size of CPUs) or other influential factors like 

Regressions in such impacting metrics, e.g. the amount of allocated memory or bytes read from an application's database,
are indicators that the application's performance has degraded.

There are some advantages over performance tests on actual latency and throughput themselves:

* You can identify potential regressions also when running tests on hardware different (less powerful) from the actual production hardware
* Tests are robust and not prone to factors like concurrent load on a CI server or a virtualized/containerized execution environment
* Certain categories of potential performance regressions can be identified also without running a production-like worklad
