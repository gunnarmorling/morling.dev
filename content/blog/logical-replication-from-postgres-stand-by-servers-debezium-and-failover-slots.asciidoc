---
title: 'Logical Replication From Postgres 16 Stand-By Servers—Debezium and Failover Slots'
...
_This post originally appeared on the link:https://www.decodable.co/blog/logical-replication-from-postgres-16-stand-by-servers-part-2-of-2[Decodable blog]. All rights reserved._
...
If you want to learn more about logical replication in general, and why and how to use it with stand-by servers in Postgres 16, then I suggest heading over to link:/blog/stand-by-servers-for-postgres-logical-replication/[part one] of this blog series before continuing here.
...
link:https://debezium.io/[Debezium], offering CDC support for many databases, including Postgres, makes use of that for exposing change event streams via Apache Kafka, but also—via its link:https://debezium.io/documentation/reference/stable/operations/debezium-server.html[Debezium Server] component—to other kinds of messaging and streaming platforms such as AWS Kinesis, Apache Pulsar, NATS, and others.
So let's quickly test how to stream changes from a Postgres stand-by with Debezium.

Note that we'll need to use the latest Debezium release—2.5.0.Beta 1, link:https://debezium.io/blog/2023/12/04/debezium-2-5-beta1-released/[released last week] —in order to stream changes from a stand-by server.
...
.Solution Overview
image::logical_replication_standby-1.png[]

Fun fact: this uses Kafka in KRaft mode, i.e. no ZooKeeper process is needed.
Good times!
...
Usually, Debezium will set up the publication—similar to the logical replication slot—automatically.
...
[NOTE]
====
Instead of the `ALL TABLES` publication, you could also narrow this further down and only expose the changes for specific tables, omit certain columns (e.g.
...
But if you're like me and tend to forget all the exact endpoint URLs, then take a look at link:https://github.com/kcctl/kcctl[kcctl] 🧸 (yes, the teddy bear emoji is part of the name), which I am going to use in the following.
It is  a command line client for Kafka Connect, which makes it very easy to create connectors, restart and stop them, etc.
...
...
----
At this point, you could hook up that change event stream with link:https://flink.apache.org/[Apache Flink], or the link:https://docs.decodable.co/docs/kafka[Decodable Kafka source connector] for feeding it into a real-time stream processing pipeline, but I'll leave that for another day 🙂.
...
Discussions around adding support for fail-over slots go back to Postgres versions as link:https://wiki.postgresql.org/wiki/Failover_slots[old as 9.6].
More recently, Patroni added their own link:https://www.postgresql.eu/events/pgconfde2022/sessions/session/3745/slides/306/Implementing%20failover%20of%20logical%20replication%20slots%20in%20Patroni.pdf[solution to the problem], and EnterpriseDB released link:https://github.com/EnterpriseDB/pg_failover_slots[pg_failover_slots], a Postgres extension for slot failover.
It remains to be seen when Postgres itself adds this feature (as hinted at in link:https://www.pgcon.org/events/pgcon_2023/sessions/session/369/slides/83/PostgreSQL%2016%20and%20beyond.pdf[this presentation], it may happen with Postgres 17).
Until then, in cases where the pg_failover_slots extension isn't available—such as on Amazon RDS—logical replication slots on stand-bys let you build your own version of failover slots.
