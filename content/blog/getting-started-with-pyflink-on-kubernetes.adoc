...
_This post originally appeared on the link:https://www.decodable.co/blog/getting-started-with-pyflink-on-kubernetes[Decodable blog]._
...
_Fig.1: Solution Overview (click to enlarge)_
...
_“build scalable batch and streaming workloads, such as real-time data processing pipelines, large-scale exploratory data analysis, Machine Learning (ML) pipelines and ETL processes”._
...
Similar to Flink's Java APIs, PyFlink comes with a link:https://nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/python/datastream_tutorial/[DataStream]  API and a link:https://nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/python/table_api_tutorial/[Table]  API._
_The former lets you implement operations such as filtering and mapping, joining, grouping and aggregation, and much more on data streams, providing you with a large degree of freedom and control over aspects such as state management, late event handling, or output control._
_In contrast, the Table API offers a more rigid but also easier-to-use and less verbose relational programming interface, including support for defining stream processing pipelines using SQL, benefitting from automatic optimizations by Flink's query planner.
...
. Deploy the link:https://github.com/cert-manager/cert-manager[certificate manager]  (required later on when the operator's webhook is invoked during the creation of custom resources): `kubectl create -f https://github.com/cert-manager/cert-manager/releases/download/v1.8.2/cert-manager.yaml`
. Add the Helm repository for the operator: `helm repo add flink-operator-repo https://downloads.apache.org/flink/flink-kubernetes-operator-1.7.0/`
. Install the operator using the provided Helm chart: `helm install flink-kubernetes-operator flink-operator-repo/flink-kubernetes-operator` +
...
----
_Listing 1: pyflink_hello_world.py (_ link:https://github.com/decodableco/examples/blob/main/pyflink/pyflink_hello_world.py[source] _)_
...
...
----
The problem is that the upstream link:https://hub.docker.com/_/flink[Flink container image]  which is used as a base image here, itself is derived from a JRE image for Java, i.e._
_it contains only a subset of all the modules provided by the Java platform._
_ link:https://github.com/alibaba/pemja[PemJa], one of PyFlink's dependencies, requires some header files which only are provided by a complete JDK, though.
...
_This is not quite ideal in terms of overall image size, and the cleaner approach would be to create a new image derived from the JDK one, but it does the trick for now to get going:
...
----
_Listing 2: Dockerfile.pyflink-base (_ link:https://github.com/decodableco/examples/blob/main/pyflink/Dockerfile.pyflink-base[source] _)_
...
----
_Listing 3: Dockerfile (_ link:https://github.com/decodableco/examples/blob/main/pyflink/Dockerfile[source] _)_
...
----
_Listing 4: pyflink-hello-world.yaml (_ link:https://github.com/decodableco/examples/blob/main/pyflink/pyflink-hello-world.yaml[source] _)_
...
_Fig._
_2: PyFlink job in the Apache Flink Dashboard (click to enlarge)_Finally, to stop your job, simply delete its resource definition:
...
----
And there you have it—Your first PyFlink job running on Kubernetes 🎉.
