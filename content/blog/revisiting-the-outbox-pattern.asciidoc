...
_This post originally appeared on the link:https://www.decodable.co/blog/revisiting-the-outbox-pattern[Decodable blog]. All rights reserved._
...
When the appointment service receives a request via its REST API, it must persist the information about the requested treatment in its own database.
...
.The Oh-my-Dawg microservices architecture
image::outbox-pattern-1.webp[]

Now, it is vital that these two operations, updating the appointment database and sending the notification, happen link:https://www.decodable.co/blog/aggregating-change-data-capture-events-based-on-transactional-boundaries[atomically--that is, they either both happen, or neither of them does].
...
.The outbox pattern for atomically updating the Appointments table and emitting a notification
image::outbox-pattern-2.webp[]

A separate process, called the outbox relay, picks up that message from this outbox table and sends it to Kafka.
...
If there are multiple transactions running in parallel, and each one emits an outbox message, it can't be guaranteed that the order of messages in the outbox table--e.g., designated by timestamps or a sequence field--is the same as the order of commits.
...
But this also means that there can be inconsistencies between the state of the writing service's local database and the external state as represented by its published messages.
While serialization may be enforced in some cases, for instance using optimistic locking on a specific record, the problem can't be solved generically in the presence of concurrent writers.

A much better solution is to retrieve link:https://rmoff.net/2024/10/15/why-do-i-need-cdc/#_the_best_way_to_do_cdc_is_log_based_cdc[the outbox events via log-based CDC].
...
If you are on Postgres, such a log-only outbox implementation is the one I'd recommend to use; you can learn more about the details in link:https://www.infoq.com/articles/wonders-of-postgres-logical-decoding-messages/[this article].
...
The outbox pattern definitely fulfills its purpose: allowing a service to update its own database and send out notifications to other services via streaming platforms such as Kafka, in a safe and reliable way, without requiring distributed transactions.
...
On the other hand, chances are you link:https://rmoff.net/2024/10/15/why-do-i-need-cdc/[need CDC anyways], in which case configuring it for also capturing outbox events doesn't change the consideration much.
...
.Joining "raw" change event streams (produced by Debezium and Flink CDC) via Flink SQL
image::outbox-pattern-3.webp[]

The big advantage of this over the outbox pattern is that no code changes to the source application itself are required.
