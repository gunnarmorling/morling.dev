...
_This post originally appeared on the link:https://www.decodable.co/blog/your-first-apache-flink-job[Decodable blog]. All rights reserved._
...
.Creating a new streaming application in Amazon MSF
image::decodable-vs-msf-1.webp[]

What follows is a mandatory second configuration step.
...
.Configuring streaming application in Amazon MSF after setting it up
image::decodable-vs-msf-2.webp[]
...
.Uploading a Flink job JAR as a Custom Pipeline to Decodable
image::decodable-vs-msf-3.webp[]
...
.Using a Custom Pipeline to customize logic within an end-to-end data flow
image::decodable-vs-msf-5.webp[]

In this scenario, change events from a Postgres database are transferred in real time to a Kafka topic as well as a fulltext search index in Elasticsearch.
