...
_This post originally appeared on the link:https://www.decodable.co/blog/get-running-with-apache-flink-on-kubernetes-2[Decodable blog]. All rights reserved._
...
spec:
  ...
  job:
    ...
    state: running
----
_Listing 1: Resource definition link:https://github.com/decodableco/examples/blob/main/flink-on-kubernetes/flink/custom-job-ha.yaml[custom-job-ha.yaml] for a custom Flink job with fault tolerance and high availability_
...
To enable Flink's link:https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/ha/kubernetes_ha/[job manager HA services], `high-availability.type` must be set to `kubernetes` and a directory for storing job manager metadata must be given via `high-availability.storageDir`.
In order for Flink to access data on S3, an S3 file system plug-in is enabled via the `ENABLE_BUILT_IN_PLUGINS` environment variable set for the Flink containers in the pod template of the resource descriptor.
Flink provides link:https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/filesystems/s3/[two plug-ins for S3]; we are using the Presto one as this is the recommended option for storing checkpoint data.
...
spec:
  ...
  savepoint: {}
----
_Listing 2: Resource definition link:https://github.com/decodableco/examples/blob/main/flink-on-kubernetes/flink/savepoint.yaml[savepoint.yaml] for manually triggering a savepoint_
...
----
_Listing 3: Resource definition link:https://github.com/decodableco/examples/blob/main/flink-on-kubernetes/logging/elastic-output.yaml[elastic-output.yaml], describing the output of the logging pipeline_
...
spec:
  ...
  globalOutputRefs:
    - es-output
----
_Listing 4: Resource definition link:https://github.com/decodableco/examples/blob/main/flink-on-kubernetes/logging/elastic-flow.yaml[elastic-flow.yaml], describing the logic of the logging pipeline_
...
.Flink job logs ingested into Elasticsearch
image::flink-on-kubernetes-3.png[]

As mentioned before, this is just scratching the surface when it comes to setting up observability infrastructure for Flink.
...
Besides logging, your observability strategy will typically also cover metrics (which are supported by Flink via a link:https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/metric_reporters/[range of metrics exporters], for instance for Prometheus, InfluxDB, and DataDog) and traces (e.g. supported via the link:https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/trace_reporters/[OpenTelemetry trace exporter]).
...
.Heimdall, a UI for managing multiple Flink jobs on one Kubernetes cluster
image::flink-on-kubernetes-4.png[]
...
When it comes to building a full-fledged Flink-based stream processing platform for an organization, there's a non-trivial amount of additional aspects to keep in mind, including the following:
