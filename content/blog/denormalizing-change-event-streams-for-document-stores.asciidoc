...
_This post originally appeared on the link:https://www.decodable.co/blog/denormalizing-change-event-streams-for-document-stores[Decodable blog]. All rights reserved._
...
.Data pipeline from RDBMS to search index with denormalization step
image::denormalize-pipeline.webp[]

In a link:https://www.youtube.com/watch?v=ICJ7-YyaC-4[Data Streaming Quick Tips episode] a few months ago, I showed how to implement a user-defined function (UDF) for Apache Flink for exactly this purpose: it takes a list of values and emits them as a strongly-typed array.
...
.Data model in the source MySQL database
image::denormalize-datamodel.webp[]
...
.Solution overview
image::denormalize-solution.webp[]

Now grab your auth token link:https://dashboard.ngrok.com/get-started/your-authtoken[from the ngrok dashboard].
...
.Discovered source tables and corresponding streams in Decodable
image::denormalize-source-streams.webp[]

Specify a name for the new connection, for instance`ecom_source`, then click "Create Connection".
...
.Running connector with output metrics
image::denormalize-connector-running.webp[]

To examine the incoming data, click on "Outbound to 3 streams", select any one of the streams and click "Run Preview":

.Preview of data ingested into Decodable
image::denormalize-preview.webp[]

When starting up, the connector automatically took an initial snapshot of the data at this point in time.
...
.SQL editor with preview of the query results
image::denormalize-array-agg-query.webp[]

Note that the fields of the array elements still use default names like `EXPR$0`.
...
.Running SQL pipeline with input and output metrics
image::denormalize-pipeline-running.webp[]

You can also take a look at the joined records in the `ecom__orders_with_lines` stream.
...
image::denormalize-sink-config.webp[]

Click "Next" and select the `ecom_orders_with_lines` stream as the source for this connection.
...
.Fuzzy query in the OpenSearch Dev Tools
image::denormalize-opensearch-query.webp[]

And it doesn't stop there; ingesting your data into a system like OpenSearch enables a wide range of analytics and dashboarding use cases.
...
.An OpenSearch dashboard for the counts of orders with items in specific categories
image::denormalize-dashboard.webp[]

This concludes this experiment for setting up a real-time data movement and processing pipeline on Decodable.
