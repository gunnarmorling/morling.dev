...
_This post originally appeared on the link:https://www.decodable.co/blog/get-running-with-apache-flink-on-kubernetes-1[Decodable blog]. All rights reserved._
...
.Solution overview
image::flink-on-kubernetes-1.png[]
...
.link:https://github.com/decodableco/examples/blob/main/flink-on-kubernetes/kind-cluster.yml[kind-cluster.yml] for spinning up a local Kubernetes cluster
...
.link:https://github.com/decodableco/examples/blob/main/flink-on-kubernetes/helm-values.yaml[helm-values.yaml] for customizing the operator configuration
[source,yaml,linenums]
----
...
In particular when deploying jobs of unknown provenience, it can make sense to use the link:https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/resource-providers/standalone/overview/[Standalone] provider instead, in which case Flink jobs don't have access to the Kubernetes API, but instead the operator is managing all the required resources.
...
.Resource definition link:https://github.com/decodableco/examples/blob/main/flink-on-kubernetes/flink/basic.yaml[basic.yaml] for a first Flink job
[source,yaml,linenums]
----
...
.The Flink web UI
image::flink-on-kubernetes-2.png[]

Next, let's make some changes to the job definition.
...
.Updated job in the Flink web UI
...
Refer to link:/blog/getting-started-with-pyflink-on-kubernetes[this post] for learning how to get started with running your first PyFlink jobs on Kubernetes.
...
.Resource definition link:https://github.com/decodableco/examples/blob/main/flink-on-kubernetes/flink/custom-job.yaml[custom-job.yaml] for a custom Flink job
[source,yaml,linenums]
----
apiVersion: flink.apache.org/v1beta1
kind: FlinkDeployment
